...
export function SimulationDashboard() {
  ...
  useEffect(() => {
    ...
    if (token) {
      fetch('http://localhost:5000/api/simulation/state', {
        headers: { 'Authorization': `Bearer ${token}` }
  ...
  const handleAction = async (action: 'Accept' | 'Reject' | 'Modify', rec: any) => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/simulation/action', {
        method: 'POST',
        headers: {
  ...
  const handleSimulateSurge = async (config: any) => {
    ...
    try {
      ...
      // Backend API call
      const response = await fetch('http://localhost:5000/api/simulation/demand-surge', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleResourceChange = (type: 'beds' | 'doctors' | 'nurses', index: number) => {
    ...
    setAfterState(newState);
       ...
       fetch('http://localhost:5000/api/simulation/resource-change', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
